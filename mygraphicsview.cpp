...
void MyGraphicsView::keyPressEvent(QKeyEvent *event)
{
    ...
    switch (event->key()) {
    case Qt::Key_Left: if (player[0]) player[0]->moveBy(-step,0); break;
    case Qt::Key_Right: if (player[0]) player[0]->moveBy(step,0); break;
    case Qt::Key_Up: if (player[0]) player[0]->moveBy(0, -step); break;
    case Qt::Key_Down: if (player[0]) player[0]->moveBy(0, step); break;
    ...
    for (QGraphicsItem* item : collisions) {
        ...
        for (int i = 0; i < 20; ++i) {
            if (player[i] == item) {
                ...
                if (chance < player[i]->stats.interception && interceptionPossible &&player[i]!=player[dernierJoueur]) {
                    // 5% par point d'interception
                ...
                else if(chance >= player[i]->stats.interception && interceptionPossible){
                        qDebug() << "Interception raté par joueur" << i;
                    interceptionPossible=false;
...
{if(!balleEnMouvement){
    ...
    if (meilleurCible != -1) {
       // equipeBalle=player[meilleurCible]->getTeam();
        ...
        dribbleAgainst(player[trouverAdversaireLePlusProche(porteur,player[20])]);
...
void MyGraphicsView::degageBalle(int porteur)
{
    double avanceX = (porteur >= 10) ? -1 : 1;
    double avanceY = 0;
    int puissanceDegagement = 500; // Distance du dégagement
